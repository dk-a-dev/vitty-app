...
interface NoteDao {
    ...
    @Query("SELECT * FROM notes WHERE courseId = :courseId AND (title LIKE '%' || :query || '%' OR content LIKE '%' || :query || '%') ORDER BY createdAt DESC")
    fun searchNotes(courseId: String, query: String): Flow<List<NoteEntity>>
}
